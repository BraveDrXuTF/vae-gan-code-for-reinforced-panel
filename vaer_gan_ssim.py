...
class VAER_GAN(keras.Model):
    ...
    def train_step(self, one_batch_data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # reconstruction_loss = reconstruction_loss*image_size*image_size 加上这两句出现负损失
            ...
            # refined_loss = refined_loss*image_size*image_size
...
def reconstructed_loss(real_img,reconstructed_img):
    real_img = tf.expand_dims(real_img,axis=-1)
    # reconstructed_img = tf.expand_dims(reconstructed_img,axis=-1)
    # print(real_img.shape)
    # print(reconstructed_img.shape)
    # 结果：
    # (None, 128, 128, 1)
    # (None, 128, 128, 1, 1)
    ssim_loss = tf.image.ssim(reconstructed_img, real_img, max_val=1, filter_size=100)
    # if ssim_loss.numpy()[0]<0:
    #     print('err')
    #     sys.exit(0)
    #  AttributeError: 'Tensor' object has no attribute 'numpy'
    real_img = tf.squeeze(real_img)
    reconstructed_img = K.flatten(reconstructed_img)
    real_img = K.flatten(real_img) # 显示shape为(None,)
    difference = reconstructed_img-real_img
    weighted_img_difference = tf.multiply(K.square(difference),1+0.2*real_img)
    return tf.multiply(ssim_loss, tf.reduce_mean(weighted_img_difference,axis=-1))
