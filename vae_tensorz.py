...
def test_plot(vae,a128batch,epoch):
    ...
    plt.savefig('images/test_tensorz530_epoch{0}.jpg'.format(epoch))
    plt.close()
##画图
...
def plot_resultsz2(models,epoch):
    ...
    filename = os.path.join('images', "digits_over_latent16w_epoch{0}tensorz530.png".format(epoch))
...
class MyepochsaveCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        vae.save_weights(os.path.join(self.save_dir,'modelepoch_{0}z2tensorz530.h5'.format(epoch)))
    def on_train_batch_end(self, batch, logs=None):
        if batch%500==1:
            vae.save_weights(os.path.join(self.save_dir,'modelbatch{0}z2tensorz530.h5'.format(batch)))
image_size = 64
...
if __name__ == '__main__':
    ...
    if args.weights:
        ...
        if args.cp2:
            ...
            vae.load_weights(filepath)
            vae.fit(parsed_image_dataset,epochs=epochs,callbacks=[MyPlotCallback_test(vae,a128batch),MyepochsaveCallback(save_dir,vae)])
            filepath = 'vae_jiajin_16wtensorz530.h5'
            vae.save_weights(filepath)
        elif args.train:
            vae.fit(parsed_image_dataset,epochs=epochs,callbacks=[MyPlotCallback_test(vae,a128batch),MyepochsaveCallback(save_dir,vae)])
            filepath = 'vae_jiajin_16wtensorz530.h5'
