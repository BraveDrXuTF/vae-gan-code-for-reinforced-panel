...
def vae_loss(x_reconstructed,x_true,z_mean,z_std):
    ...
    encode_decode_loss = tf.pow(x_reconstructed- x_true,2)
    encode_decode_loss = tf.reduce_mean(encode_decode_loss)
    #KL损失
    kl_div_loss=1+z_std-tf.square(z_mean)-tf.exp(z_std)
    kl_div_loss=-0.05*tf.reduce_sum(kl_div_loss,1)
...
with tf.Session() as sess:
    ...
    ###在原来基础上继续训练的语句
    # saver.restore(sess, cwd+"model\\D16_guiyi_Final_pic_neta-4100")
    for i in range(train_num+1):
        ...
        x_batch.shape
        xuexilv = 0.01
        sess.run(trainer,feed_dict={x_:x_batch[:,:,:,[0]] ,  targets_ :x_batch[:,:,:,[0]],starter_learning_rate : xuexilv})
        if i % prin_num == 0:
             ...
             fig.savefig(cwd + 'VAE_XTF_train_pic\\VAE_D16_guiyi_First_pic%.5d.jpg'%i,dpi=100)
             plt.close()
             
             saver.save(sess,"I:\\zhangkunpeng\\bian_Cjin\\XTF_model\\VAE_D16_guiyi_First_neta", global_step=i)
