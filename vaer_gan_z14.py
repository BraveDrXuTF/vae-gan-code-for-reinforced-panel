...
def build_discriminator_with_teacher(filters=16):
    ...
    # z_teacher = Dropout(rate=0.6)(z_teacher_input)  # z_teacher具有0.6的概率死掉
    # 这里dropout是不是不太好
    z_teacher = z_teacher_input
    z_embedding = Dense(960, activation='linear',
                        name='z_embbding_dis')(z_teacher)
    z_embedding = tf.keras.layers.PReLU(alpha_initializer='zeros', alpha_regularizer=None,alpha_constraint=None, shared_axes=None)(z_embedding)
    # 3层卷积
    for i in range(3):
        filters *= 2
        x = Conv2D(filters=filters,
    ...
    x = Dense(960, activation='gelu', kernel_initializer=tf.keras.initializers.GlorotUniform())(x)
...
def build_refiner():
    ...
    # 首先实验简单的线性变换
    # z_dropout = Dropout(p=0.1)(z)
    z_dropout = z
    z_embedding = Dense(image_size*image_size, activation='linear')(z_dropout)
...
class VAER_GAN(keras.Model):
    ...
    def train_step(self, one_batch_data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            ref_about_discriminator_loss = self.ref_about_discriminator_loss(
                refined_reconstructed_output, refined_generated_output)
            # 难道我一直忽略gen_about_discriminator_loss + ref_about_discriminator_loss这两项？
            vae_loss = reconstruction_loss + kl_loss + refined_loss + \
                gen_about_discriminator_loss + ref_about_discriminator_loss
        ...
        refiner_gradients2 = tape.gradient(
            ref_about_discriminator_loss, self.refiner.trainable_variables)
...
def reconstructed_loss(real_img, reconstructed_img):
    ...
    weighted_img_difference = tf.multiply(K.square(difference), 1+0.2*real_img)
...
# 编译VAER_GAN
VAER_GAN_instance.compile(encoder_optimizer, generator_optimizer, discriminator_optimizer, refiner_optimizer,
                          reconstructed_loss, kl_loss, discriminator_loss, gen_about_discriminator_loss, ref_about_discriminator_loss)
...
def test_plot(model, a128batch, epoch):
    ...
    plt.figure(figsize=(15, 15))
    plt.imshow(figure, cmap='Greys_r')
    plt.savefig('images/test_vaer_ld14_epoch{0}_{1}.jpg'.format(
        epoch, time.strftime("_%a_%b_%d_%H_%M_%S_%Y", time.localtime())))
    plt.close()
...
class MyepochsaveCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        self.vae.save_weights(os.path.join(self.save_dir, 'modelepoch_{0}z2vaer_ld14_GAN{1}.h5'.format(
            epoch, time.strftime("_%a_%b_%d_%H_%M_%S_%Y", time.localtime()))))
...
if __name__ == '__main__':
    ...
    # VAER_GAN_instance.fit(parsed_image_dataset, epochs=epochs, callbacks=[MyPlotCallback_test(
