...
from vae_gan_transformer_scheme1 import TransformerEncoder
# from vae_gan_transformer_scheme2 import TransformerEncoder
...
# import vae_gan_transformer_scheme2
...
def parse_imagestr2numpy(image_features):
    ...
    image2 = image2[:,:,0:1] #把这一行去掉就可以实现三通道图片的输入
    return image2
# def testFunc():
...
class VAER_GAN(keras.Model):
    ...
    def train_step(self, one_batch_data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            vae_loss = reconstruction_loss + kl_loss + refined_loss + gen_about_discriminator_loss + ref_about_discriminator_loss
...
# encoder_optimizer = tf.keras.optimizers.SGD(1e-4, momentum=0.9) 效果不好，不过对学习策略可以继续探索
...
def test_plot(model,a128batch,epoch):
    ...
    for i in range(col):
        # import pdb
        # pdb.set_trace()
        figure[0:digit_size,i * digit_size: (i + 1) * digit_size] = a128batch_list[0][i][:,:,0]
        figure[1*digit_size:2*digit_size,i * digit_size: (i + 1) * digit_size] = testresult[i]
        figure[2*digit_size:3*digit_size,i * digit_size: (i + 1) * digit_size] = a128batch_list[0][i + int(batch_size/2)][:,:,0]
...
class MyepochsaveCallback(Callback):
    def __init__(self, save_dir, vae):
        ...
        self.vae=vae
    def on_epoch_end(self, epoch, logs=None):
        self.vae.save_weights(os.path.join(self.save_dir,'modelepoch_{0}z2vaerGAN{1}'.format(epoch,time.strftime("_%a_%b_%d_%H_%M_%S_%Y", time.localtime()))))
         

if __name__ == '__main__':
    print(1)
    parsed_image_dataset
    a128batch = parsed_image_dataset.take(2)
