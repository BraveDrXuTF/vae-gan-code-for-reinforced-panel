...
def test_plot(model,a128batch,epoch):
    ...
    plt.figure(figsize=(15, 15))
    plt.imshow(figure, cmap='Greys_r')
    plt.savefig('images/test_GAN614_epoch_lowconf{0}.jpg'.format(epoch))
    plt.close()
...
class MyepochsaveCallback(Callback):
    def __init__(self, save_dir, vae):
        ...
        self.vae=vae
    def on_epoch_end(self, epoch, logs=None):
        self.vae.save_weights(os.path.join(self.save_dir,'modelepoch_{0}z2GAN614lowconf.h5'.format(epoch)))
