...
def plot_resultsz2(models,epoch):
    ...
    plt.savefig(filename)
    plt.close() #这句话保证图像不会重叠
def plot_KL_test_results(data,vae,epoch):
    # filename = os.path.join('images', "KL_{0}_night.png".format(epoch))
    filename = os.path.join('images', "KL_batch{0}_noon.png".format(epoch))
...
class MyPlotCallbackKL(Callback):
    def __init__(self, data, vae):
        self.data = data
        self.vae = vae
    def on_train_batch_end(self, batch, logs=None):
        if batch%100==1:
            plot_KL_test_results(self.data, vae, batch)
class MyepochsaveCallback(Callback):
    def __init__(self, save_dir, vae):
        self.save_dir = save_dir
    def on_epoch_end(self, epoch, logs=None):
        vae.save_weights(os.path.join(self.save_dir,'modelepoch_{0}KL.h5'.format(epoch)))
    def on_train_batch_end(self, batch, logs=None):
        if batch%100==1:
            vae.save_weights(os.path.join(self.save_dir,'modelbatch{0}KL_noon.h5'.format(batch)))
        if batch%50==1:
...
x = Flatten()(x) # 16384
#z是隐含向量
#z_mean和z_log_var都是同维度的batch_size*1024
#这两个参数是针对每一个z分量具有的高斯分布
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cpKL", help=help_)
    args = parser.parse_args()
    models = (encoder, decoder)
    # data = (x_test, y_test)

    # VAE loss = 三部分 稀疏变分自编码
    if args.bce:
    ...
    kl_loss *= -0.5
    sparsity_level = 0.1
    z_hat = K.mean(z,axis=0) # 沿着batch_size求平均，批次求平均，求完后应为1*latent_dim
    sparsity_loss = sparsity_level * (K.log(sparsity_level) - K.log(K.clip(z_hat,1e-8,float("inf")))) + (1-sparsity_level)*(K.log(1-sparsity_level) - K.log(K.clip(1-z, 1e-8, float("inf"))))
    sparsity_loss = K.sum(sparsity_loss)
    vae_loss = K.mean(reconstruction_loss + kl_loss) + sparsity_loss
    # import pdb
    # pdb.set_trace()
    ...
    if args.weights:
        ...
        if args.cp2:
            ...
            vae.fit(parsed_image_dataset,epochs=epochs,callbacks=[MyPlotCallbackKL(a128batch,vae),MyepochsaveCallback(save_dir,vae)])
            filepath = 'vae_jiajin_KL.h5'
            vae.save_weights(filepath)
        elif args.cpKL:
            filepath = os.path.join(save_dir, args.cpKL)
            vae.load_weights(filepath)
            vae.fit(parsed_image_dataset,epochs=epochs,callbacks=[MyPlotCallbackKL(a128batch,vae),MyepochsaveCallback(save_dir,vae)])
            filepath = 'vae_jiajin_KL.h5'
            vae.save_weights(filepath)
