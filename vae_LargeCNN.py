...
def plot_resultsz2(models,epoch):
    ...
    filename = os.path.join('images', "digits_over_latent16w_epoch{0}_random524524.png".format(epoch))
...
class MyepochsaveCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        vae.save_weights(os.path.join(self.save_dir,'modelepoch_{0}z2LC_random524.h5'.format(epoch)))
    def on_train_batch_end(self, batch, logs=None):
        if batch%500==1:
            vae.save_weights(os.path.join(self.save_dir,'modelbatch{0}z2LC_random524.h5'.format(batch)))
image_size = 64
tfrecord_list=glob.glob('data/*.tfrecords')
# random.shuffle(tfrecord_list) # suijsuiji
...
if __name__ == '__main__':
    ...
    if args.weights:
        ...
        if args.cp2:
            ...
            filepath = 'vae_jiajin_16wLC_random524.h5'
            vae.save_weights(filepath)
        elif args.cpz2LC:
