...
def plot_resultsz2(models,epoch):
    ...
    filename = os.path.join('images', "digits_over_latent16w_epoch{0}LC3524.png".format(epoch))
...
class MyepochsaveCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        vae.save_weights(os.path.join(self.save_dir,'modelepoch_{0}z2LC3524.h5'.format(epoch)))
    def on_train_batch_end(self, batch, logs=None):
        if batch%500==1:
            vae.save_weights(os.path.join(self.save_dir,'modelbatch{0}z2LC3524.h5'.format(batch)))
image_size = 64
...
if __name__ == '__main__':
    ...
    if args.weights:
        ...
        if args.cp2:
            ...
            vae.load_weights(filepath)
            vae.fit(parsed_image_dataset,epochs=epochs,callbacks=[MyPlotCallbackz2(models),MyepochsaveCallback(save_dir,vae)])
            filepath = 'vae_jiajin_16wLC3524.h5'
            vae.save_weights(filepath)
        elif args.train:
            vae.fit(parsed_image_dataset,epochs=epochs,callbacks=[MyPlotCallbackz2(models),MyepochsaveCallback(save_dir,vae)])
            filepath = 'vae_jiajin_16wLC3524.h5'
