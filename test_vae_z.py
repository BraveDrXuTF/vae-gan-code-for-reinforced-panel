...
def plot_results(models, batch):
    ...
    for z3 in z3_list:
        filename = os.path.join(
            'images', "digits_over_latent16w_ind{0}_z3is{1}.png".format(
                batch // 500 + 1, z3))
...
def plot_resultsz2(models, epoch):
    ...
    filename = os.path.join(
        'images',
        "digits_over_latent16w_epoch{0}GAN614.png".format(epoch))
    ...
    def train_step(self, one_batch_data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            reconstructed_output = self.discriminator(
                reconstructed_img, training=True)
            ...
            discriminator_loss = self.discriminator_loss(
                real_output, reconstructed_output, generated_output)

            vae_loss = reconstruction_loss + kl_loss
            gen_about_discriminator_loss = self.gen_about_discriminator_loss(
                reconstructed_output, generated_output)
...
def reconstructed_loss(real_img, reconstructed_img):
    ...
    weighted_img_difference = tf.multiply(
        K.square(difference), 0.9 + 2 * real_img)
    return image_size * image_size * \
        tf.reduce_mean(weighted_img_difference, axis=-1)
...
def discriminator_loss(real_output, reconstructed_output, generated_output):
    ...
    loss3 = keras.losses.BinaryCrossentropy(
        from_logits=True, reduction=tf.keras.losses.Reduction.NONE)(
...
vae_gan_instance = VAE_GAN(
    generator_instance,
    discriminator_instance,
    encoder_instance)
# vae_gan_instance.load_weights('vae_cnn_weights/modelepoch_39z2GAN614.h5')
# 编译vae_gan
vae_gan_instance.compile(
    encoder_optimizer,
    generator_optimizer,
    discriminator_optimizer,
    reconstructed_loss,
...
def test_plot(model, a128batch, epoch):
    ...
    for i in range(col):
        ...
        figure[2 * digit_size:3 * digit_size, i *
               digit_size: (i + 1) * digit_size] = a128batch_list[0][i + 10]
        figure[3 * digit_size:4 * digit_size, i *
               digit_size: (i + 1) * digit_size] = testresult[i + 10]
    plt.figure(figsize=(15, 15))
    plt.imshow(figure, cmap='Greys_r')
...
class MyepochsaveCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        self.vae.save_weights(
            os.path.join(
                self.save_dir,
                'modelepoch_{0}z2GAN614.h5'.format(epoch)))
    ...
    def on_train_batch_end(self, batch, logs=None):
        if batch % 500 == 1:
            self.vae.save_weights(
                os.path.join(
                    self.save_dir,
                    'modelbatch{0}z2GAN614.h5'.format(batch)))
...
if __name__ == '__main__':
    ...
    # pass
    # a = vae_gan_instance(a128batch)
    # vae_gan_instance.build(input_shape = input_shape)
    vae_gan_instance.built = True
    vae_gan_instance.load_weights('vae_cnn_weights/modelepoch_38z2GAN614.h5')
    # z_try = np.ones((1,latent_dim,latent_dim,1))
    # gen_predict = vae_gan_instance.generator(z_try)
    # gen_predict = np.squeeze(gen_predict)

    # plt.imshow(gen_predict, cmap='Greys_r')
    # plt.show()
    z_mean, z_log_var, z_a128 = vae_gan_instance.encoder.predict(a128batch)
    z_a128 = np.squeeze(z_a128)
    # print(z_a128)
    plt.imshow(z_a128[1], cmap='Greys_r')
    plt.show()
    # vae_gan_instance.generator()
