...
def test_plot(vae,a128batch,epoch):
    ...
    plt.figure(figsize=(15, 15))
    plt.imshow(figure, cmap='Greys_r')
    plt.savefig('images/test_resnet_low1_epoch{0}.jpg'.format(epoch))
    plt.close()
...
class MyepochsaveCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        vae.save_weights(os.path.join(self.save_dir,'modelepoch_{0}z2LC_resnet_low1.h5'.format(epoch)))
    def on_train_batch_end(self, batch, logs=None):
        if batch%500==1:
            vae.save_weights(os.path.join(self.save_dir,'modelbatch{0}z2LC_resnet_low1.h5'.format(batch)))
num_filters = 16
image_size = 64
...
if __name__ == '__main__':
    ...
    if args.weights:
        ...
        if args.cp2:
            ...
            filepath = 'vae_jiajin_16wLC_resnet_low1.h5'
            vae.save_weights(filepath)
        elif args.cpz2LC:
